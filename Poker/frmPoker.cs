...
namespace Poker
{
    public partial class frmPoker : Form
    {
        ...
        /// <summary>
        /// 每局開始前的按鈕狀態：必須先押注，才能發牌。
        /// </summary>
        private void ResetRoundButtons()
        {
            btnDealCard.Enabled = false;
            btnChangeCard.Enabled = false;
            btnCheck.Enabled = false;

            bool canBet = totalMoney > 0;
            btnBet.Enabled = canBet;
            txtBetAmount.Enabled = canBet;
        }

        /// <summary>
        /// 一局結束後，清除本局押注並回到等待押注狀態。
        /// </summary>
        ...
        private void btnBet_Click(object sender, EventArgs e)
        {
            ...
            if (bet > totalMoney)
            {
                MessageBox.Show("押注金額不可大於目前總資金。", "下注錯誤", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                txtBetAmount.Focus();
                return;
            }
            ...
            lblResult.Text = $"本局押注：{currentBet}，請按「發牌」。";
        }
        ...
        private void btnCheck_Click(object sender, EventArgs e)
        {
            ...
            int winMoney = currentBet * payoutRate;
            totalMoney += winMoney;
            ShowMoney();

            if (payoutRate > 0)
            {
                lblResult.Text = $"{result}\r\n賠率：{payoutRate} 倍，中獎金額：{winMoney}，目前總資金：{totalMoney}";
            }
            else
            {
                lblResult.Text = $"{result}\r\n沒有中獎，損失押注：{currentBet}，目前總資金：{totalMoney}";
            }
